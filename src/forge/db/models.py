...
class Track(str, enum.Enum):
    """Which side of the market a need hypothesis addresses (dual-track)."""

    customer = "customer"  # a firm that HAS the problem
    producer = "producer"  # a firm that would MAKE/sell the solution


class NeedHypothesis(Base):
    """A FALSIFIABLE hypothesis that a company has a need this asset could serve.

    Generated by matching an asset's candidate applications against the
    Relationship Graph. It is only ever a HYPOTHESIS (rule 4): it becomes a real
    need only after a human validation records ``need_confirmed``. Every hypothesis
    carries the evidence it was derived from (grounding) and a strength from the
    convergence of graph tie + shared market terms + corroborating signals. One row
    per (asset, company, track), replaced on re-generation.
    """

    __tablename__ = "need_hypothesis"
    __table_args__ = (
        UniqueConstraint("asset_id", "company_id", "track", name="uq_need_hypothesis"),
        Index("ix_need_hypothesis_asset", "asset_id"),
    )
    ...
    hypothesised_need: Mapped[str] = mapped_column(Text, nullable=False)
    track: Mapped[Track] = mapped_column(Enum(Track, name="track"), nullable=False)
    strength: Mapped[Strength] = mapped_column(
        Enum(Strength, name="strength"), nullable=False
    )
    # [{kind, ref, detail}] — what grounds the hypothesis (graph tie, terms, signal).
